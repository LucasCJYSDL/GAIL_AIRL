...
def traj_segment_generator(pi, env, path_num, stochastic, style_index):
        ...
        while True:
            ...
            if new:
                ...
                path = {"ob": np.array(obs), "new": np.array(news), "ac": np.array(acs), "mean": np.array(means), "logstd": np.array(logstds), "vpred": np.array(vpreds),
                 "true_reward": np.array(true_rews), "sparse_reward": np.array(sparse_rews), "nextvpred": vpred * (1 - new), "prevac": np.array(prevacs)}
                paths.append(path)
                break
