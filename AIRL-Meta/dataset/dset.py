...
class AutoDrive_Dset(object):
    def __init__(self, expert_path, train_fraction=0.7, traj_limitation=-1, randomize=True):
        ...
        self.means = self.acs
        std_acs = np.std(np.array(self.acs), axis=0)

        self.logstds = []
        #print(self.logstds)
        for i in range(len(self.means)):
            self.logstds.append(np.log(std_acs))

        self.logstds = np.array(self.logstds)
...
def test(expert_path, traj_limitation, plot):
    dset = AutoDrive_Dset(expert_path, traj_limitation=traj_limitation)
    a,b,c,d = dset.get_next_batch(10)
    if plot:
        dset.plot()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--expert_path", type=str, default="/home/path/PycharmProjects/AIRL-Lane_Change/data/AutoDrive-v0.AutoDrive-v0.npz")
