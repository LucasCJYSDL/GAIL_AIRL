...
def learn(env, policy_func, dataset, optim_batch_size=128, max_iters=1e4, adam_epsilon=1e-5, optim_stepsize=3e-4,
    ...
    ac = U.get_placeholder("true_ac", dtype=tf.int64, shape=(None, ))
    stochastic = U.get_placeholder_cached(name="stochastic")
    ac_onehot = tf.one_hot(ac, pi.pdtype.param_shape()[0], on_value=1, off_value=None, axis=1)
    ac_onehot = tf.cast(ac_onehot, tf.float32)
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=ac_onehot, logits=pi.pdparam))
    ...
    for iter_so_far in tqdm(range(int(max_iters))):
        ob_expert, ac_expert, _, _ = dataset.get_next_batch(optim_batch_size)
        train_loss, g = lossandgrad(ob_expert, ac_expert, True)
        adam.update(g, optim_stepsize)
        logger.log("Itrations: {} Training loss: {}".format(iter_so_far, train_loss))
