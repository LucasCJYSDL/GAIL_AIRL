...
from train_util import allmean, timed, traj_segment_generator, add_vtarg_and_adv, compute_path_probs
...
def learn(env, policy_func, reward_giver, expert_dataset, rank,
          ...
          vf_stepsize=3e-4, d_stepsize=3e-5, vf_iters=3,
    ...
    while True:
        ...
        ob, ac, mean, std, lprobs = seg["ob"], seg["ac"], seg["mean"], seg["logstd"], seg["lprobs"]
        ...
        for ob_batch, ac_batch, lprobs_batch in dataset.iterbatches((ob, ac, lprobs),
            ...
            *newlosses, g = reward_giver.lossandgrad(ob_batch, ac_batch, ob_expert, ac_expert, labels, lprob_batch)
            d_adam.update(allmean(g, nworkers), d_stepsize)
            d_losses.append(newlosses)
        logger.log(fmt_row(13, np.mean(d_losses, axis=0)))

        # ------------------ get Reward and Advantage ------------------
        ob_expert, ac_expert, mean_expert, std_expert = expert_dataset.get_next_batch(len(ob))
        lprobs_expert = compute_path_probs(ac_expert, mean_expert, std_expert, ac_space, is_expert=True)
        horizon = len(ob)
        lprobs_rew = np.concatenate([lprobs, lprobs_expert], axis=0)
        ...
        if use_true_reward:
            seg["rew"] = rewards = reward_giver.get_reward(ob, ac, ob_expert, ac_expert, horizon, lprobs_rew) + true_rew ##reward_2

            print("1")

            print(sum(reward_giver.get_reward(ob, ac, ob_expert, ac_expert, horizon, lprobs_rew)))
            ...
            seg["rew"] = rewards = reward_giver.get_reward(ob, ac, ob_expert, ac_expert, horizon,
                                                           lprobs_rew) + 10 * sparse_rew  ##reward_2
            ...
            print(sum(sparse_rew))

        else:
            seg["rew"] = rewards = reward_giver.get_reward(ob, ac, ob_expert, ac_expert, horizon, lprobs_rew)  ##reward_1

        #debug_info = reward_giver.debug(ob, ac, ob_expert, ac_expert, labels, lprobs_rew)
